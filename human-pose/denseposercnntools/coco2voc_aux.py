...
def annsToSeg(anns, coco_instance):
    ...
    class_sep_seg = np.zeros((len(masks), h, w))
    ...
    for i, mask in enumerate(masks):
        ...
        class_sep_seg[i] = np.where(class_sep_seg[i]>0, class_sep_seg[i], mask*anns[i]['category_id'])

    return class_seg, instance_seg, id_seg.astype(np.int64), class_sep_seg
